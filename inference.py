...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for position in self.legalPositions:
            self.particles += [position]* particlesPerPosition
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        particlePos = list(itertools.product(self.legalPositions,repeat=self.numGhosts))
        random.shuffle(particlePos)
        print "particles: ",particlePos
        count = 0
        self.particles = []
        while count < self.numParticles:
            for position in particlePos:
